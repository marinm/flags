function is_nonzero_number(tile) {
	return (
		tile && !tile.hidden && [1, 2, 3, 4, 5, 6, 7, 8].includes(tile.value)
	);
}
...
function is_unknown(tile) {
	return (
		tile &&
		tile.hidden &&
		!tile.is_inferred_number &&
		!tile.is_inferred_flag
	);
}
...
function select_next_unrevealed_flag({ gamestate, selectTile, socket }) {
	const hidden_tiles = gamestate.board.filter(
		(tile) => tile.is_inferred_flag
	);
